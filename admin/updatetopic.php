...
<body>
    <h2>Update Topic</h2>
    <div class="first">
            <form action="updatetopic.php" method="GET" enctype="multipart/form-data">
                ...
                <input type="text" name="web" placeholder=" Enter a website link" value="<?php echo $_GET['web'];?>"><br><br>
                <label>Youtube video : </label><br>
                <input type="text" name="vid" placeholder="Enter a video link" value="<?php echo $_GET['vid'];?>"><br><br>
...
<?php
    if(isset($_GET["submit"])){
        ...
        $video=$_GET["vid"];                  
        $id=$_GET["id"];
        $sql ="UPDATE topic SET coursecode='$coursecode',course='$subject',chaptername='$chapter',topic='$topic',tutorial='$video',website='$website' WHERE id='$id'";

        if(mysqli_query($conn,$sql)){
            echo"<script>alert('You have successfully updated the data');</script>";
            ?>
            <META HTTP-EQUIV="Refresh" CONTENT ="0; URL= http://localhost/noteapp/admin/topic.php?code=<?php echo $coursecode ?>">
            <?php
        }
        else{
