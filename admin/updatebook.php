...
<body>
    ...
    <div class="first">
            <form action="updatebook.php" method="POST" enctype="multipart/form-data">                
                ...
                <input type="hidden" name="oldbook" value="<?php echo $row['book']; ?>"><br><br>
...
<?php
    if(isset($_POST["update"]))
    {
        ...
        $course=$_POST["c"];      
        $id=$_POST["id"];
        $newbook=$_FILES["newbook"]["name"];
        $oldbook=$_POST["oldbook"];

        if($newbook != '')
        {
            $up_book = $_FILES["newbook"]["name"];
        }
        else{
            $up_book = $oldbook;
        }
        ...
        if($_FILES["newbook"]["name"] != '')
        {
            if(file_exists($upload_dir.'/'.$_FILES["newbook"]["name"]))
            {
                $sql1 ="UPDATE book SET coursecode='$coursecode',course='$course',book='$up_book' WHERE id='$id'";
            ...
            {
                move_uploaded_file($_FILES["newbook"]["tmp_name"],$upload_dir.'/'.$_FILES["newbook"]["name"]);
                unlink($upload_dir.'/'.$oldslide);
                $sql ="UPDATE book SET coursecode='$coursecode',course='$course',book='$up_book' WHERE id='$id'";
